...
import (
	...
	"restaurant-service/internal/repository/postgres"
...
func WithPostgresStore(driverName, dataSourceName string) Configuration {
	return func(s *Repository) (err error) {
		...
		if err = store.Migrate(driverName, dataSourceName); err != nil {
			return
		}

		s.Restaurant = postgres.NewRestaurantRepository(s.postgres.Client)
